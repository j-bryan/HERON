...
class TemplateDriver(Base):
  ...
  def write_workflow(self, loc, case, components, sources):
    ...
    print('========================')
    self._template.writeWorkflow(loc, case, components, sources)
...
class Template(TemplateBase, Base):
  ...
  def _build_result_statistic_names(case, components):
    ...
    # Variable names for total activity for all component-resource interactions
    tot_act_vars = []
    for component in components:
      for tracker in component.get_tracking_vars():
        resource_list = np.sort(list(component.get_resources()))
    ...
    # Econ metric names
    for e_metric in econ_metrics+tot_act_vars:  # for all econ metric names and total activity variable names...
      for name in result_statistics:  # for all result statistic names...
        out_name = case.stats_metrics_meta[name]['prefix']
        # do I need to add percent or threshold?
        if name in ['percentile', 'valueAtRisk', 'expectedShortfall', 'sortinoRatio', 'gainLossRatio']:  # add an additional attrib field to name if one of these
          # Builds names of the form <abbrev stat name>_<result metric name>_<
          # multiple percents or thresholds may be specified
          # NOTE: result_statistics[name] is sometimes None (e.g. for all default stats metrics)
          #       If not None, it is a threshold or percent value or list of threshold/percent values
          if isinstance(result_statistics[name], list):
            for attrib in result_statistics[name]:
              names.append(out_name+'_'+attrib+'_'+e_metric)  # example: perc_5_NPV, perc_95_NPV
          else:
            names.append(out_name+'_'+result_statistics[name]+'_'+e_metric)  # example: sortino_median_NPV
        else:  # result_statistic doesn't have any additional attributes (percents, thresholds), just the name
          out_name += '_'+e_metric  # example: mean_NPV, std_NPV, max_NPV
