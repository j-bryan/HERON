...
class SamplerVariable(RavenSnippet):
  ...
  def set_sampling_strategy(self, construction="equal", type="CDF", steps: int | None = None, values: list[float] = [0, 1]) -> None:
    ...
    grid_node.set("construction", _to_string(construction))
    grid_node.set("type", _to_string(type))
    if construction == "equal":
      grid_node.set("steps", _to_string(steps))
    grid_node.text = _to_string(values, delimiter=" ")
